...
def Modelrun(corr_w,Qyh, Qyc, injectionT, Thmin):
    ...
    subsurffile = 'sub-surface.csv'     #load the correct subsurface settings
    monitoringfile = 'monitoring.csv'   #load the correct monitoring settings (optional, not used in this script)
    ...
    mon_obj_list = createobjfromCSV(PySystem,monitoringfile)
    ...
    for i in range(len(well_obj_list)):
        if well_obj_list[i].T_inj > Tmax:
            Tmax = well_obj_list[i].T_inj # reset maximum injection temperature, in case well type is incorectly defined
        if well_obj_list[i].T_inj < Tmin:
            Tmin = well_obj_list[i].T_inj # reset minimum injection temperature, in case incorrectly defined earlier
    ...
    for period in range(rl):
        ...
        for j in range(len(well_obj_list)):
            ...
            if well_obj_list[j].type == 'warm':
                ...
                Run_output.loc[period,'Dens_water_in'] = denseref + drhodT * well_obj_list[j].T_inj
                Run_output.loc[period,'Dens_water_out'] = denseref + drhodT * well_obj_list[j].T_modflow
                Run_output.loc[period,'Efficiency_c'] = corr_c
